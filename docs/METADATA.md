...
- `path` and `module` appear when the corresponding buildinfo lines do, plus one entry per `build KEY=VALUE` setting with `-` stripped from the key. Observed on a Go 1.26 arm64/darwin build: `-buildmode=exe` → `"buildmode": "exe"` and `-compiler=gc` → `"compiler": "gc"`, while environment-style keys keep their spelling unchanged (`"CGO_ENABLED": "1"`, `"GOARCH": "arm64"`, `"GOOS": "darwin"`, `"GOARM64": "v8.0"`). A module-less build (`go build main.go`) reports `"path": "command-line-arguments"`.
- The key is present on every binary parsed by the ELF/PE/Mach-O readers — `{}` for non-Go binaries (observed `{}` on `/bin/ls`) — so an empty object means "not a Go binary or no readable buildinfo", not "Go with no build settings". On PE the blob is scanned out of the first 1/32 of the `.data` section (the linker places it near the segment start); the same byte-level version read applies there.
