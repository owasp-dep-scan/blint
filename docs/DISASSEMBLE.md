...
| `instruction_lengths`         | List of Integers   | Per-instruction sizes in bytes, aligned one-to-one with the `assembly` lines. A consumer can reconstruct any line's address by prefix-summing lengths from its CFG block's `start` VA; per block, the summed lengths must equal `end` - `start`.                                                   |
...
| `has_crypto_call`             | Boolean            | True if the function's disassembly text contains patterns indicating cryptographic operations (based on `blint.lib.indicators.CRYPTO_INDICATORS`).                                                                                                                                                 |
| `has_gpu_call`                | Boolean            | True if the function's disassembly text contains patterns indicating GPU-related operations (based on `blint.lib.indicators.GPU_INDICATORS`).                                                                                                                                                      |
...
## Function Discovery for Stripped Binaries
