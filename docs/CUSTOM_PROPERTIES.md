...
These properties come from vendored-source banner detection: version strings a vendored library copy leaves in the binary (for example `deflate 1.3.1 Copyright ...`). Every signature requires the version to appear inside a string that also names the library, so a bare version-like string is never a banner. A matched banner becomes a component only when the artifact corroborates it — it defines symbols from that library's own API — or when it declares no dynamic dependencies at all (a stripped static image has no symbol table to corroborate with, and nowhere else for the code to be). A banner-shaped string in a dynamically linked artifact defining none of the API is a *mention* (a stale build-time banner naming a version that is not even the linked one), recorded on the parent component rather than emitted as a dependency. Banner evidence is independent of blintdb; a banner for a library blintdb already attributed is recorded on that component as corroboration instead of a duplicate.
...
These properties come from the ELF ABI analysis. Most sit on the parent component and describe the binary as a whole. The last three sit on the library components recovered for runtime-loaded dependencies. See [`abi_analysis`](METADATA.md#abi_analysis) for how the values are derived.
