...
| `internal:abi_provider`            | library component | single value | On every component derived from an ABI requirement | The version node provider the component represents, for example `GLIBC` or `GLIBCXX`.                                                                                          |
| `internal:abi_symbol_count`        | library component | number like  | On every component derived from an ABI requirement | How many imported symbols bind to this provider, which indicates how deeply the binary depends on it.                                                                          |
| `internal:abi_determining_symbols` | library component | comma list   | When the component carries a version               | The imported symbols that set the component's version. Use these to trace an unexpectedly high requirement back to the single import responsible.                              |
...
The version on an ABI component is a **floor**, not the installed version. `pkg:generic/gnu/libc@2.28` means "glibc 2.28 or newer is required", so a consumer matching it against a vulnerability database should treat it as a lower bound rather than an exact version.
