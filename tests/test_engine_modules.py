...
class TestCanonicalCfgHash:
    ...
    def test_dense_graph_hashes_in_bounded_time(self):
        """Refinement colors must stay fixed-width.

        Carrying raw nested signatures forward grows each color by a factor
        of its out-degree per round, so a few-hundred-block function with
        branching blocks becomes tens of kilobytes per block and dominates
        the parse. A dense 400-block graph is the regression canary.
        """
        import time
        ...
        graph = {
            "blocks": [
                {"start": hex(0x1000 + i * 4), "end": hex(0x1004 + i * 4), "instructions": i % 7}
                for i in range(block_count)
            ],
            "edges": [
                {"src": i, "dst": (i + step) % block_count, "kind": "jump"}
                for i in range(block_count)
                for step in (1, 2, 3)
            ],
        }
        started = time.perf_counter()
        digest = function_cfg_hash(graph)
        assert len(digest) == 16
        assert time.perf_counter() - started < 1.0

    def test_refinement_separates_graphs_sharing_aggregates(self):
        """Two graphs with identical counts but different wiring must differ."""

        def graph(edges):
            return {
                "blocks": [
                    {"start": hex(i * 4), "end": hex(i * 4 + 4), "instructions": 2}
                    for i in range(4)
                ],
                "edges": [{"src": s, "dst": d, "kind": "jump"} for s, d in edges],
            }

        chain = graph([(0, 1), (1, 2), (2, 3)])
        star = graph([(0, 1), (0, 2), (0, 3)])
        assert function_cfg_hash(chain) != function_cfg_hash(star)
