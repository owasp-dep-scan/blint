...
def gate_sbom(corpus: Path, workdir: Path, seeds: list[str]) -> None:
    ...
    # The parallelism claim is *within* a seed: jobs=4 must equal the
    # sequential jobs=1 run under the same PYTHONHASHSEED. Cross-seed
    # variance of the sequential output is reported separately as the
    # pre-existing property it is (the sequential dependsOn order derives
    # from set iteration, which is seed-dependent).
    gate_failed = False
    for seed in seeds:
        ...
        if par == seq:
            ...
            gate_failed = True
    if len({snapshots[(s, "1")] for s in seeds}) > 1:
        print(
            "  note: the sequential SBOM output itself varies with PYTHONHASHSEED "
            "(dependsOn order derives from set iteration) — pre-existing, "
            "identical for both sequential legs of a given seed"
        )
    if gate_failed:
        raise SystemExit("sbom parallel != sequential within a seed")
