...
def _assert_replay_equals_cold(warm: dict, cold: dict) -> None:
    ...
    """
    cold_export_domain = orjson.loads(serialize_metadata(cold))
...
def cache_options(tmp_path):
    """BlintOptions pointed at an isolated reports dir, cache explicitly on.

    The cache is opt-in, so every cache test must ask for it the way a caller
    does — which also means the rest of the suite exercises the uncached path
    by default.
    """
    return BlintOptions(
        src_dir_image=[],
...
def test_digest_derived_from_parse_signature(cache_options):
    ...
    cache_options.use_cache = not cache_options.use_cache
    assert compute_options_digest(cache_options) == base
...
def test_parse_cache_off_unless_opted_in(cache_options):
    """Without --cache there is no cache instance and the coverage block says so."""
    cache_options.use_cache = False
    runner = AnalysisRunner()
...
def test_cli_cache_flag_default_off():
    from blint.cli import build_parser

    args = build_parser().parse_args(["--cache", "-i", "/tmp"])
...
def test_uncacheable_metadata_is_skipped_not_mangled(tmp_path):
    """A value orjson cannot serialize makes the entry unstoreable.

    The alternative — an orjson ``default`` of ``str`` — would store
    ``"b'\\x00'"`` where the export path writes hex, so a replay would differ
    from a reparse in exactly the way the byte-identity gate is meant to
    catch, but silently and only for a field no fixture happens to have.
    Refusing the store keeps the cache correct-or-absent.
    """
    cache = ParseCache(cache_dir=str(tmp_path / "store"))
    assert cache.put("bb" * 32, "digest", {"binary_type": "ELF", "raw": b"\x00\xff"}) is False
    assert cache.get("bb" * 32, "/some/path", "digest") is None
    cache.close()


def test_findings_and_reviews_match_cold_run(native_binary, tmp_path):
    """A warm run's findings and reviews must equal a cold run's.

    The metadata byte-identity gate compares the exported dict, but checks
    and reviews run on the *returned* object, which on a hit carries plain
    ints where a cold parse holds live LIEF enums. This runs the real
    pipeline with reviews enabled — the path the other cache tests disable —
    so a divergence in that domain cannot hide.
    """

    def run(reports_dir: Path) -> tuple[list, list]:
        options = BlintOptions(
            src_dir_image=[str(native_binary)],
            reports_dir=str(reports_dir),
            use_cache=True,
        )
        runner = AnalysisRunner()
        findings, reviews, _, _ = runner.start(options, [str(native_binary)])
        return findings, reviews, runner.cache_hits

    cold_findings, cold_reviews, cold_hits = run(tmp_path / "cold")
    warm_findings, warm_reviews, warm_hits = run(tmp_path / "warm")
