...
def _certificate_names(certificate_content: bytes) -> dict:
    ...
    The outer Certificate TLV is already consumed by the caller; the content
    starts with TBSCertificate. Its field order after the optional [0]
    version: serialNumber, signature (algorithm), issuer, validity, subject —
    the second and fourth SEQUENCE are therefore the issuer and subject
    Names.
    """
    names = {"subject_cn": None, "subject_organization": None, "issuer_cn": None}
    try:
        pos = 0
        end = len(certificate_content)
        sequence_count = 0
        while pos < end:
            tag, value, pos = _ber_read(certificate_content, pos, end)
...
def _certificate_serial(certificate_content: bytes) -> str | None:
    ...
    Like :func:`_certificate_names`, takes the SEQUENCE content with the
    outer TLV already consumed; the serial is the first untagged INTEGER
