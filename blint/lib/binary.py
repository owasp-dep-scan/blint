...
from blint.lib.codesign_macho import parse_superblob, signature_summary
...
def _macho_slice_signature(exe_file: str, parsed_slice: lief.MachO.Binary) -> dict:
    ...
    try:
        ...
        if code_signature is None:
            return {"available": False, "parse_status": "absent"}
        blob, _blob_source = _macho_signature_blob(exe_file, code_signature)
        if not blob:
...
def _macho_signature_data_offset(code_signature) -> int | None:
    """File offset of the signature blob (the load command's ``dataoff``).

    LIEF 1.0 exposes no offset attribute on the code signature; the 16 bytes
    its ``data`` property returns *are* the LC_CODE_SIGNATURE load command
    ``(cmd, cmdsize, dataoff, datasize)``, so the offset is read back out of
    them, trying the slice's byte order both ways.
    """
...
def _macho_signature_blob(exe_file: str, code_signature) -> tuple[bytes | None, str]:
    """The raw SuperBlob bytes, and where they came from.

    Primary source is LIEF's ``content`` (it reads the range the load command
    names). When that is empty, the file range is read directly using the
    offset recovered by :func:`_macho_signature_data_offset` — the caller
    records which source produced the bytes.
    """
...
def _macho_code_signature_block(exe_file: str, code_signature) -> dict:
    ...
    confident "unsigned" or an empty entitlements answer. ``data_offset`` and
    ``blob_source`` say where the real SuperBlob lives and which read path
    produced the bytes. The legacy ``size``/``data_size`` keys keep their
    ...
    if not block["available"]:
        return block
    blob, blob_source = _macho_signature_blob(exe_file, code_signature)
...
def add_mach0_signature(exe_file: str, metadata: dict, parsed_obj: lief.MachO.Binary) -> dict:
    ...
    try:
        ...
        if code_signature is None:
            ...
            metadata["code_signature"] = _macho_code_signature_block(exe_file, code_signature)
