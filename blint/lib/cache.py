...
def _reject_unserializable(obj: Any) -> Any:
    """orjson ``default`` that refuses rather than guesses.

    The export path serializes metadata with ``json_serializer``, which turns
    ``bytes`` into utf-8 or hex and applies the export-time
    ``BLINT_MAX_HEX_BYTES`` truncation. The cache must not use it: truncation
    is export-time and deliberately outside the cache key, so a cached
    truncated value would replay under a different budget. It must not use
    ``str`` either — that would silently store ``"b'\\x00...'"`` where a cold
    parse exports hex, making a replay differ from a reparse in a way no test
    catches until some future field holds a non-JSON type.

    So: anything orjson cannot serialize natively makes the entry
    unstoreable. Nothing in ``parse()`` output reaches this today (the
    corpus byte-identity gate proves it); if something ever does, the result
    is not cached instead of being cached wrong.
    """
    raise TypeError(f"parse metadata holds an uncacheable {type(obj).__name__}")


def serialize_metadata(metadata: dict) -> bytes:
    """Serialize parse metadata for the cache; raises on anything unsupported.

    Exposed so the byte-identity tests normalize the cold side through the
    exact function the store uses, rather than a second copy of it.
    """
    return orjson.dumps(metadata, default=_reject_unserializable)


def _blint_version() -> str:
    """The running blint version; part of the cache key.

    This is the *installed distribution's* version, which is what invalidates
    the cache across releases. In an editable/development checkout it is
    whatever was recorded at ``pip install -e`` time, so it does not move when
    the working tree changes: a developer editing ``binary.py`` gets stale
    replays until they reinstall or run ``blint cache clear``. That is
    survivable because the cache is opt-in (``--cache``) — an accidental
    development run does not touch it at all.
    """
...
class ParseCache:
    ...
    def put(self, file_sha256: str, options_digest: str, metadata: dict) -> bool:
        ...
        if not all(key in metadata for key in _STORE_MIN_KEYS):
            ...
            return False
        try:
            raw = serialize_metadata(metadata)
            data = zlib.compress(raw, _ZLIB_LEVEL)
        except (TypeError, ValueError) as exc:
            LOG.debug("Not caching parse result: %s", exc)
            return False
