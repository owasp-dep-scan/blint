...
def run_pool(
    ...
    # its queue but before it read it; that undelivered unit is already
    # accounted for as a hard failure, so a replacement inheriting the same
    # queue would pick up stale work and desynchronize the slot's in-flight
    # bookkeeping — after which a second death would blame the wrong unit.
    ...
    def spawn(slot: int) -> None:
        ...
        spawns_allowed -= 1
        stale = task_queues.pop(slot, None)
        if stale is not None:
            # Discard anything the dead worker never read rather than letting
            # the replacement inherit it.
            with contextlib.suppress(Exception):
                stale.cancel_join_thread()
                stale.close()
        task_queues[slot] = ctx.Queue()
        proc = ctx.Process(
            target=worker_main,
