        ...
        for entry in metadata.get(bucket) or []:
            ...
            if separator and is_library_name(library):
                ...
                if provides:
                    ...
                    # Every unconfirmed symbol is collected, not just the
                    # first sample-cap worth: the count is the signal an
                    # analyst acts on ("the SDK could not vouch for 140 of
                    # these"), so capping collection would make the count
                    # saturate at the cap and understate the gap. Only the
                    # sample below is bounded.
