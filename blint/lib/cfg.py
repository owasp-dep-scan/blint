    ...
    for block_index, (start, end) in enumerate(block_bounds):
        ...
        if kind == "jump":
            ...
            if target_index is not None:
                ...
                # The target address is not stored: a branch target is always
                # a block leader, so it is exactly the destination block's
                # ``start`` and repeating it here only inflates the metadata.
                edges.append(
                    {
                        "src": block_index,
                        "dst": target_block,
