...
- For extraction logic, use the per-format readers `blint/lib/binary_{elf,pe,macho,wasm}.py` (shared helpers in `binary_common.py`, entry point and callgraph in `binary.py`) and `blint/lib/android.py`.
